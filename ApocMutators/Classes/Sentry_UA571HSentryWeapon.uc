class Sentry_UA571HSentryWeapon extends KFWeapon
    Config(ApocMutators);
...
var Sentry_UA571HSentry CurrentSentry;
...
var Class<Sentry_UA571HSentry> SentryClass;
...
simulated function Destroyed()
{
    ...
    {
        // Hack to switch to another weapon on the client when we throw the Sentry_UA571HSentry bot is destroyed.
    ...
    if (CurrentSentry == None)
    {
        ...
        if (FastTrace(Spot, Instigator.Location))
        {
            if (bStationaryTurret)
                CurrentSentry = Spawn(Class'Sentry_UA571HSentryStationary', , , Spot, R);
            else
                CurrentSentry = Spawn(Class'Sentry_UA571HSentry', , , Spot, R);
            if (CurrentSentry != None)
            {
                if (PlayerController(Instigator.Controller) != None)
                    PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Sentry_UA571HSentryMessageTurret', 1);
        ...
        if (PlayerController(Instigator.Controller) != None)
            PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Sentry_UA571HSentryMessageTurret', 0);
    }
}
...
defaultproperties
{
     SentryClass=Class'ApocMutators.Sentry_UA571HSentry'
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'ApocMutators.Sentry_UA571HSentryPickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'ApocMutators.Sentry_UA571HSentryAttachment'
