class Sentry_USCMSentryWeapon extends KFWeapon
    Config(ApocMutators);
...
var Sentry_USCMSentry CurrentSentry;
...
var Class<Sentry_USCMSentry> SentryClass;
...
simulated function Destroyed()
{
    ...
    {
        // Hack to switch to another weapon on the client when we throw the Sentry_USCMSentry bot is destroyed.
    ...
    if (CurrentSentry == None)
    {
        ...
        if (FastTrace(Spot, Instigator.Location))
        {
            if (bStationaryTurret)
                CurrentSentry = Spawn(Class'Sentry_USCMSentryStationary', , , Spot, R);
            else
                CurrentSentry = Spawn(Class'Sentry_USCMSentry', , , Spot, R);
            if (CurrentSentry != None)
            {
                if (PlayerController(Instigator.Controller) != None)
                    PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Sentry_USCMSentryMessageTurret', 1);
        ...
        if (PlayerController(Instigator.Controller) != None)
            PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Sentry_USCMSentryMessageTurret', 0);
    }
}
...
defaultproperties
{
     SentryClass=Class'ApocMutators.Sentry_USCMSentry'
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'ApocMutators.Sentry_USCMSentryPickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'ApocMutators.Sentry_USCMSentryAttachment'
