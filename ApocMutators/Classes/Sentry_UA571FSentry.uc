// Written by Marco
class Sentry_UA571FSentry extends SVehicle// Gay hack, but has to be done.
    Placeable
    Config(ApocMutators)
    CacheExempt;
...
var transient Sentry_UA571FSentryMuzzleFlash Flashes[4];
...
var Pawn OwnerPawn;
var Sentry_UA571FSentryWeapon WeaponOwner;
...
final function SetOwningPlayer(Pawn Other, Sentry_UA571FSentryWeapon Wep)
{
    OwnerPawn = Other;
...
simulated function PostRender2D(Canvas C, float ScreenLocX, float ScreenLocY)
{
    ...
    if (C.Viewport.Actor.PlayerReplicationInfo == PlayerReplicationInfo)
        C.SetDrawColor(100, 100, 255, 255);
    ...
    if (Level.NetMode != NM_Client && Physics == PHYS_Karma)
    {
        ...
        if (KParams != None && KParams.bContactingLevel && NextFlipCheckTime < Level.TimeSeconds)
        {
            ...
            if (bFlip != bIsCurrentlyFlipped)
            {
                ...
                Sentry_UA571FSentryAI(Controller).NotifyGotFlipped(bFlip);
            }
        }
    }
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, Class<DamageType> damageType, optional int HitIndex)
{
    ...
    if (Health <= 0)
    {
        ...
        Sentry_UA571FSentryAI(Controller).NotifyGotFlipped(true);
    }
}
...
simulated function Timer()
{
    ...
    if ((Level.TimeSeconds-LastRenderTime) < 4)
    {
        ...
        {
            Flashes[FXBarrelNumber] = Spawn(Class'Sentry_UA571FSentryMuzzleFlash');
            AttachToBone(Flashes[FXBarrelNumber], BarrelBones[FXBarrelNumber]);
            Flashes[FXBarrelNumber + 1] = Spawn(Class'Sentry_UA571FSentryMuzzleFlash');
...
function Died(Controller Killer, Class<DamageType> damageType, vector HitLocation)
{
    ...
    if (WeaponOwner != None)
    {
        if (OwnerPawn != None && PlayerController(OwnerPawn.Controller) != None)
            PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'Sentry_UA571FSentryMessageTurret', 2);
...
defaultproperties
{
     ...
     HitDamage=37
     SentryHealth=1200
     HitDgeType=Class'ApocMutators.Sentry_DamTypeUA571FSentry'
     ...
     MenuName="UA-571F Turret"
     ControllerClass=Class'ApocMutators.Sentry_UA571FSentryAI'
     bStasis=False
     ...
     End Object
     KParams=KarmaParamsRBFull'ApocMutators.Sentry_UA571FSentry.KarmaParamsRBFull0'
     bBlockActors=False
}
