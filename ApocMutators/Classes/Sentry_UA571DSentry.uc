// Written by Marco
class Sentry_UA571DSentry extends SVehicle// Gay hack, but has to be done.
    Placeable
    Config(ApocMutators)
    CacheExempt;
...
var transient Sentry_UA571DSentryMuzzleFlash Flashes[4];
...
var Pawn OwnerPawn;
var Sentry_UA571DSentryWeapon WeaponOwner;
...
final function SetOwningPlayer(Pawn Other, Sentry_UA571DSentryWeapon Wep)
{
    OwnerPawn = Other;
...
simulated function PostRender2D(Canvas C, float ScreenLocX, float ScreenLocY)
{
    ...
    if (C.Viewport.Actor.PlayerReplicationInfo == PlayerReplicationInfo)
        C.SetDrawColor(255, 255, 100, 255);
    ...
    if (Level.NetMode != NM_Client && Physics == PHYS_Karma)
    {
        ...
        if (KParams != None && KParams.bContactingLevel && NextFlipCheckTime < Level.TimeSeconds)
        {
            ...
            if (bFlip != bIsCurrentlyFlipped)
            {
                ...
                Sentry_UA571DSentryAI(Controller).NotifyGotFlipped(bFlip);
            }
        }
    }
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, Class<DamageType> damageType, optional int HitIndex)
{
    ...
    if (Health <= 0)
    {
        ...
        Sentry_UA571DSentryAI(Controller).NotifyGotFlipped(true);
    }
}
...
simulated function Timer()
{
    ...
    if ((Level.TimeSeconds-LastRenderTime) < 4)
    {
        ...
        {
            Flashes[FXBarrelNumber] = Spawn(Class'Sentry_UA571DSentryMuzzleFlash');
            AttachToBone(Flashes[FXBarrelNumber], BarrelBones[FXBarrelNumber]);
            Flashes[FXBarrelNumber + 1] = Spawn(Class'Sentry_UA571DSentryMuzzleFlash');
...
function Died(Controller Killer, Class<DamageType> damageType, vector HitLocation)
{
    ...
    if (WeaponOwner != None)
    {
        if (OwnerPawn != None && PlayerController(OwnerPawn.Controller) != None)
            PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'Sentry_UA571DSentryMessageTurret', 2);
...
defaultproperties
{
     ...
     HitDamage=37
     SentryHealth=1200
     HitDgeType=Class'ApocMutators.Sentry_DamTypeUA571DSentry'
     FireRateTime=0.700000
     ...
     MenuName="UA-571D Turret"
     ControllerClass=Class'ApocMutators.Sentry_UA571DSentryAI'
     bStasis=False
     ...
     End Object
     KParams=KarmaParamsRBFull'ApocMutators.Sentry_UA571DSentry.KarmaParamsRBFull0'
     bBlockActors=False
}
