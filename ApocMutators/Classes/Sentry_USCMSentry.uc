// Written by Marco
class Sentry_USCMSentry extends SVehicle// Gay hack, but has to be done.
    Placeable
    Config(ApocMutators)
    CacheExempt;
...
var transient Sentry_USCMSentryMuzzleFlash Flashes[4];
...
var Pawn OwnerPawn;
var Sentry_USCMSentryWeapon WeaponOwner;
...
final function SetOwningPlayer(Pawn Other, Sentry_USCMSentryWeapon Wep)
{
    OwnerPawn = Other;
...
simulated function PostRender2D(Canvas C, float ScreenLocX, float ScreenLocY)
{
    ...
    if (C.Viewport.Actor.PlayerReplicationInfo == PlayerReplicationInfo)
        C.SetDrawColor(135, 206, 250, 255);
    else C.SetDrawColor(255, 100, 255);
    ...
    if (Level.NetMode != NM_Client && Physics == PHYS_Karma)
    {
        ...
        if (KParams != None && KParams.bContactingLevel && NextFlipCheckTime < Level.TimeSeconds)
        {
            ...
            if (bFlip != bIsCurrentlyFlipped)
            {
                ...
                Sentry_USCMSentryAI(Controller).NotifyGotFlipped(bFlip);
            }
        }
    }
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, Class<DamageType> damageType, optional int HitIndex)
{
    ...
    if (Health <= 0)
    {
        ...
        Sentry_USCMSentryAI(Controller).NotifyGotFlipped(true);
    }
}
...
simulated function Timer()
{
    ...
    if ((Level.TimeSeconds-LastRenderTime) < 4)
    {
        ...
        {
            Flashes[FXBarrelNumber] = Spawn(Class'Sentry_USCMSentryMuzzleFlash');
            AttachToBone(Flashes[FXBarrelNumber], BarrelBones[FXBarrelNumber]);
            Flashes[FXBarrelNumber + 1] = Spawn(Class'Sentry_USCMSentryMuzzleFlash');
...
function Died(Controller Killer, Class<DamageType> damageType, vector HitLocation)
{
    ...
    if (WeaponOwner != None)
    {
        if (OwnerPawn != None && PlayerController(OwnerPawn.Controller) != None)
            PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'Sentry_USCMSentryMessageTurret', 2);
...
defaultproperties
{
     ...
     HitDamage=37
     SentryHealth=1200
     HitDgeType=Class'ApocMutators.Sentry_DamTypeUSCMSentry'
     ...
     ControllerClass=Class'ApocMutators.Sentry_USCMSentryAI'
     bStasis=False
     ...
     End Object
     KParams=KarmaParamsRBFull'ApocMutators.Sentry_USCMSentry.KarmaParamsRBFull0'
     bBlockActors=False
}
