// Written by Marco
Class Sentry_SentryTurret extends SVehicle// Gay hack, but has to be done.
    Placeable
    Config(ApocMutators)
    CacheExempt;

#exec obj load file="PortalTurret.ukx" package="ApocMutators"

var transient Sentry_SentryTurretMuzzleFlash Flashes[4];
var transient byte BarrelNumber, FXBarrelNumber;
var transient float NextVoiceTimer;
var transient Sentry_SentryTurretLazor LaserFX;
...
var Pawn OwnerPawn;
var Sentry_SentryTurretWeapon WeaponOwner;
...
final function SetOwningPlayer(Pawn Other, Sentry_SentryTurretWeapon Wep)
{
    OwnerPawn = Other;
...
simulated function PostRender2D(Canvas C, float ScreenLocX, float ScreenLocY)
{
    ...
    if (C.Viewport.Actor.PlayerReplicationInfo == PlayerReplicationInfo)
        C.SetDrawColor(100, 255, 100, 255);
    ...
    if (Level.NetMode != NM_Client && Physics == PHYS_Karma)
    {
        ...
        if (KParams != None && KParams.bContactingLevel && NextFlipCheckTime < Level.TimeSeconds)
        {
            ...
            if (bFlip != bIsCurrentlyFlipped)
            {
                ...
                Sentry_SentryTurretAI(Controller).NotifyGotFlipped(bFlip);
            }
        }
    }
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, Class<DamageType> damageType, optional int HitIndex)
{
    ...
    if (Health <= 0)
    {
        ...
        Sentry_SentryTurretAI(Controller).NotifyGotFlipped(true);
    }
}
simulated function PlayDeploy()
{
    ...
    if (Level.NetMode == NM_DedicatedServer)
        Return;
    if (LaserFX == None)
        LaserFX = Spawn(Class'Sentry_SentryTurretLazor', Self);
...
simulated function Timer()
{
    ...
    PlaySound(Sound'TShootClose', SLOT_Pain, 0.7f, , 800.f);
    ...
    if ((Level.TimeSeconds-LastRenderTime) < 4)
    {
        ...
        {
            Flashes[FXBarrelNumber] = Spawn(Class'Sentry_SentryTurretMuzzleFlash');
            AttachToBone(Flashes[FXBarrelNumber], BarrelBones[FXBarrelNumber]);
            Flashes[FXBarrelNumber + 1] = Spawn(Class'Sentry_SentryTurretMuzzleFlash');
...
function Died(Controller Killer, Class<DamageType> damageType, vector HitLocation)
{
    ...
    if (WeaponOwner != None)
    {
        if (OwnerPawn != None && PlayerController(OwnerPawn.Controller) != None)
            PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'Sentry_SentryTurretMessage', 2);
...
{
    if (Sentry_SentryTurret(other) != None && VSizeSquared(Velocity) > VSizeSquared(other.Velocity))
        Speak(1);
}
...
defaultproperties
{
     ...
     AlarmNoiseSnd=Sound'ApocMutators.Base.Talert'
     DiedSnd=Sound'ApocMutators.Base.Tdie'
     LockedOnSnd=Sound'ApocMutators.Base.Tactive'
     Voices(0)=SoundGroup'ApocMutators.Speech.GTurretActivate'
     Voices(1)=SoundGroup'ApocMutators.Speech.GTurretCollide'
     Voices(2)=SoundGroup'ApocMutators.Speech.GTurretDisabled'
     Voices(3)=SoundGroup'ApocMutators.Speech.GTurretFF'
     Voices(4)=SoundGroup'ApocMutators.Speech.GTurretPickup'
     Voices(5)=SoundGroup'ApocMutators.Speech.GTurretSearching'
     Voices(6)=SoundGroup'ApocMutators.Speech.GTurretSleep'
     Voices(7)=SoundGroup'ApocMutators.Speech.GTurretTipped'
     HitDamage=5
     SentryHealth=400
     HitDgeType=Class'ApocMutators.Sentry_DamTypeSentryTurret'
     ...
     Health=400
     MenuName="Turret"
     ControllerClass=Class'ApocMutators.Sentry_SentryTurretAI'
     bStasis=False
     Mesh=SkeletalMesh'ApocMutators.TurretMesh'
     Skins(0)=Combiner'ApocMutators.Skins.JTurretA'
     ...
     End Object
     KParams=KarmaParamsRBFull'ApocMutators.Sentry_SentryTurret.KarmaParamsRBFull0'
     bBlockActors=False
}
