class Sentry_UA571DSentryWeapon extends KFWeapon
    Config(ApocMutators);
...
var Sentry_UA571DSentry CurrentSentry;
...
var Class<Sentry_UA571DSentry> SentryClass;
...
simulated function Destroyed()
{
    ...
    {
        // Hack to switch to another weapon on the client when we throw the Sentry_UA571DSentry bot is destroyed.
    ...
    if (CurrentSentry == None)
    {
        ...
        if (FastTrace(Spot, Instigator.Location))
        {
            if (bStationaryTurret)
                CurrentSentry = Spawn(Class'Sentry_UA571DSentryStationary', , , Spot, R);
            else
                CurrentSentry = Spawn(Class'Sentry_UA571DSentry', , , Spot, R);
            if (CurrentSentry != None)
            {
                if (PlayerController(Instigator.Controller) != None)
                    PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Sentry_UA571DSentryMessageTurret', 1);
        ...
        if (PlayerController(Instigator.Controller) != None)
            PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Sentry_UA571DSentryMessageTurret', 0);
    }
}
...
defaultproperties
{
     SentryClass=Class'ApocMutators.Sentry_UA571DSentry'
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'ApocMutators.Sentry_UA571DSentryPickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'ApocMutators.Sentry_UA571DSentryAttachment'
