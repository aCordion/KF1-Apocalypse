class FreezerNitroProjectile extends ShotgunBullet;
...
simulated function Landed( vector HitNormal )
{
    ...
    if ( Level.DetailMode >= DM_High )
        Spawn(class'FreezerNitroGroundEffect',self,,Location);    
}
	...
	if ( KFHumanPawn(Instigator) != none && !Level.bDropDetail ) {
		if ( EffectIsRelevant(Location,false) ) {
            ...
            if ( Level.DetailMode > DM_Low ) {
                Spawn(class'FreezerNitroSplash',self,,HitLocation,rotator(HitNormal));
                if ( Level.DetailMode >= DM_SuperHigh )
                    Spawn(class'FreezerNitroImpact',self,,HitLocation,rotator(-HitNormal));
            }
		}
	}
...
defaultproperties
{
     Trail1Class=Class'FreezerNitroTrail'
     Trail2Class=Class'FreezerNitroTrailB'
     MaxPenetrations=1
     ...
     MyDamageType=Class'FreezerNitroDamType'
     ImpactSound=SoundGroup'KF_EnemiesFinalSnd.Bloat.Bloat_AcidSplash'
     ExplosionDecal=Class'FreezerNitroDecal'
     DrawType=DT_None
     StaticMesh=None
