//=============================================================================
// Sentry_SentryTurretWeapon.
//=============================================================================
class Sentry_SentryTurretWeapon extends KFWeapon
    Config(ApocMutators);

var Sentry_SentryTurret CurrentSentry;
...
simulated function Destroyed()
{
    ...
    {
        // Hack to switch to another weapon on the client when we throw the Sentry_SentryTurret bot is destroyed.
    ...
    if (CurrentSentry == None)
    {
        ...
        if (FastTrace(Spot, Instigator.Location))
        {
            if (bStationaryTurret)
                CurrentSentry = Spawn(Class'Sentry_SentryTurretStationary', , , Spot, R);
            else CurrentSentry = Spawn(Class'Sentry_SentryTurret', , , Spot, R);
            if (CurrentSentry != None)
            {
                if (PlayerController(Instigator.Controller) != None)
                    PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Sentry_SentryTurretMessage', 1);
        ...
        if (PlayerController(Instigator.Controller) != None)
            PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'Sentry_SentryTurretMessage', 0);
    }
}
...
{
     MagCapacity=1
     HudImage=Texture'ApocMutators.Icons.I_Turret'
     SelectedHudImage=Texture'ApocMutators.Icons.I_Turret'
     Weight=1.000000
     bModeZeroCanDryFire=True
     TraderInfoTexture=Texture'ApocMutators.Icons.I_Turret'
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'ApocMutators.Sentry_SentryTurretPickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'ApocMutators.Sentry_SentryTurretAttachment'
     IconCoords=(X1=245,Y1=39,X2=329,Y2=79)
     ItemName="Sentry Turret"
     DrawType=DT_StaticMesh
     StaticMesh=StaticMesh'ApocMutators.pickups.PTurretMesh'
     Skins(0)=Combiner'ApocMutators.Skins.JTurretB'
     AmbientGlow=35
}
