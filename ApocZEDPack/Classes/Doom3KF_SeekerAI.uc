Class Doom3KF_SeekerAI extends Doom3KF_Doom3Controller;
...
function ExecuteWhatToDoNext()
{
	...
	else if (Doom3KF_Seeker(Pawn).Mother==None)
		GoToState('DeathWander');
	else
	{
		Enemy = Doom3KF_Seeker(Pawn).Mother.Controller.Enemy;
...
{
	if (Level.Game.bGameEnded || Doom3KF_Seeker(Pawn).Mother==None || Doom3KF_Seeker(Pawn).Mother.Controller==None || Enemy==Doom3KF_Seeker(Pawn).Mother.Controller.Enemy)
		return false;
	Enemy = Doom3KF_Seeker(Pawn).Mother.Controller.Enemy;
	return true;
}
...
state ZombieHunt
{
	function PickDestination()
	{
		...
		if (Doom3KF_Seeker(Pawn).Mother != None && FindBestPathToward(Doom3KF_Seeker(Pawn).Mother, true, true))
			return;
