class Doom3KF_Mancubus extends Doom3KF_DoomMonster;
...
{
	if (Level.NetMode != NM_DedicatedServer)
		Spawn(class'Doom3KF_RevenantSmokePuffs', , , GetBoneCoords('lmissile').Origin);
}

simulated function RFireGlow()
{
	if (Level.NetMode != NM_DedicatedServer)
		Spawn(class'Doom3KF_RevenantSmokePuffs', , , GetBoneCoords('rmissile').Origin);
}
...
defaultproperties
{
     ...
     BurnClass=Class'Doom3KF_MancubusBurnTex'
     FootstepSndRadius=800.000000
     RangedProjectile=Class'Doom3KF_MancubusProjectile'
     HasHitAnims=True
     BigMonster=True
     aimerror=200
