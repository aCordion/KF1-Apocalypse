class Doom3KF_CyberDemon extends Doom3KF_DoomMonster;
...
var transient float NextRocketTime;
...
simulated function PostBeginPlay()
{
    ...
    {
        FireEffects[0] = Spawn(class'Doom3KF_CyberDemonMouthEffect', self);
        AttachToBone(FireEffects[0], 'cyber_mouth_fire');
        FireEffects[1] = Spawn(class'Doom3KF_CyberDemonMouthSideFire', self);
        AttachToBone(FireEffects[1], 'LFire');
        FireEffects[2] = Spawn(class'Doom3KF_CyberDemonMouthSideFire', self);
...
function RangedAttack(Actor A)
{
    ...
    if (!bHasRoamed)
        RoamAtPlayer();
    else if (NextRocketTime<Level.TimeSeconds || IsInMeleeRange(A))
    {
        PrepareStillAttack(A);
        if (Health>1000)
            NextRocketTime = Level.TimeSeconds+2.f+FRand()*6.f;
        else NextRocketTime = Level.TimeSeconds+0.6f+FRand()*3.f;
...
simulated function FireProjectile()
{
    ...
    if (Level.NetMode != NM_DedicatedServer)
    {
        Spawn(class'Doom3KF_RevenantSmokePuffs', , , BoneLocation.Origin);
...
defaultproperties
{
     ...
     RangedProjectile=Class'Doom3KF_CyberDemonProjectile'
