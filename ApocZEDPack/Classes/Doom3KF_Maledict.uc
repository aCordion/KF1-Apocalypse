class Doom3KF_Maledict extends Doom3KF_DoomMonster;
...
simulated function SpawnBigMeteor() //or firewall
{
    ...
    if (Level.NetMode != NM_Client)
    {
        ...
        if (FRand()<0.5)
            RangedProjectile = Class'Doom3KF_MaledictMeteorBigProj';
        FireProj(Location);
...
function SummonMeteors()
{
    ...
    if (Controller != None && Controller.Target != None)
    {
        ...
        for (i=Rand(3); i<8; i++)
        {
            ...
            Spawn(class'Doom3KF_MaledictMeteorProj', , , HL, rotator(TargetDir+VRand()*0.01f));
        }
    }
}
...
function SummonSouls()
{
    ...
    if (Level.NetMode != NM_Client)
    {
        ...
        Spawn(Class'Doom3KF_DemonSpawnB', , , ForgottenPos);
        SetTimer(0.8, false);
        bSoulIsOK = true;
    }
}
...
function Timer()
{
    ...
    bSoulIsOK = false;
    F = Spawn(Class'Doom3KF_Forgotten', , , ForgottenPos);
    if (F != None)
    {
        NumSouls++;
        F.MSummoner = Self;
...
defaultproperties
{
     ...
     BurnClass=Class'Doom3KF_MaledictBurnTex'
     RangedProjectile=Class'Doom3KF_MaledictFireWallProj'
     ...
     AirSpeed=500.000000
     HealthMax=7500.00000 //6000.000000 kyan
     Health=7500 //6000 kyan
