class Doom3KF_Sentry extends Pawn
	Config(ApocMonsters);
...
var Emitter mTracer;
var Doom3KF_SentryWeaponFlash WeaponFlash;
var transient float NextVoiceTimer;
var Doom3KF_SentryGun WeaponOwner;
...
final function SetOwningPlayer(Pawn Other, Doom3KF_SentryGun W)
{
	OwnerPawn = Other;
...
simulated function SetAnimationNum(byte Num)
{
	...
	case 2:
		...
		{
			WeaponFlash = Spawn(Class'Doom3KF_SentryWeaponFlash', Self);
...
function Timer()
{
	...
	foreach TraceActors(Class'Actor', Res, HL, HN, Location+X*8000.f, Location)
	{
		if (Res != Self && (Res==Level || Res.bBlockActors || Res.bProjTarget || Res.bWorldGeometry) && KFPawn(Res)==None
			 && KFBulletWhipAttachment(Res)==None && Doom3KF_Sentry(Res)==None)
		{
			A = Res;
			break;
		}
	...
	if (A != None && KFPawn(A)==None)
		A.TakeDamage(HitDamage, OwnerPawn, HL, X*1000.f, Class'Doom3KF_DamTypeSentryFire');
	else if (A==None)
...
function Died(Controller Killer, class<DamageType> damageType, vector HitLocation)
{
	...
	if (WeaponOwner != None)
	{
		if (OwnerPawn != None && PlayerController(OwnerPawn.Controller) != None)
			PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'Doom3KF_SentryMessage', 2);
...
simulated function HurtRadius(float DamageAmount, float DamageRadius, class<DamageType> DamageType, float Momentum, vector HitLocation)
{
	...
	foreach CollidingActors (class 'Actor', Victims, DamageRadius, HitLocation)
	{
		...
		if ((Victims != self) && (Victims.Role == ROLE_Authority) && !Victims.IsA('FluidSurfaceInfo')
		 && ExtendedZCollision(Victims)==None && KFPawn(Victims)==None && Doom3KF_Sentry(Victims)==None)
		{
...
defaultproperties
{
     ...
     EyeHeight=0.000000
     Health=250
     ControllerClass=Class'Doom3KF_SentryController'
     bPhysicsAnimUpdate=True
