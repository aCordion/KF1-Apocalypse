class Doom3KF_Sabaoth extends Doom3KF_DoomMonster;

var transient float NextRangedTime, MultiAttackTime;
var(Sounds) Sound Laughs[3], PreFireSound;
var Doom3KF_SabaothChargeEffect Charge;
...
function RangedAttack(Actor A)
{
    ...
    else if (NextRangedTime<Level.TimeSeconds || MultiAttackTime>Level.TimeSeconds)
    {
        if (MultiAttackTime>Level.TimeSeconds)
            NextRangedTime = Level.TimeSeconds+4.f;
        else
        {
            if (FRand()<0.2f)
                MultiAttackTime = Level.TimeSeconds+2.f+FRand()*5.f;
            NextRangedTime = Level.TimeSeconds+2.f+FRand()*5.f;
        }
...
simulated function ChargeBFG()
{
    if (Level.NetMode != NM_DedicatedServer)
    {
        Charge = Spawn(class'Doom3KF_SabaothChargeEffect', , , GetBoneCoords('mech_bfg').Origin);
...
defaultproperties
{
     ...
     FadeClass=Class'Doom3KF_SabaothMaterialSequence'
     RangedProjectile=Class'Doom3KF_SabaothProjectile'
