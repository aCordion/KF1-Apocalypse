Class Doom3KF_LostSoulAI extends Doom3KF_Doom3Controller;
...
function DoCharge()
{
	if (Pawn != None)
	{
		if (Enemy != None && ActorReachable(Enemy) && !IsInState('ChargeIn') && Doom3KF_LostSoul(Pawn).ReadyToCharge(Enemy))
			GoToState('ChargeIn');
...
Ignores Timer, NotifyBump, SeePlayer, HearNoise, DamageAttitudeTo, GetOutOfTheWayOfShot;
	...
	function Tick(float Delta)
	{
		...
		if (A != None)
		{
			if (Pawn(A) != None && Doom3KF_LostSoul(A)==None)
				A.TakeDamage(Doom3KF_LostSoul(Pawn).ChargeHitDamage, Pawn, Normal(Pawn.Location-A.Location)*A.CollisionRadius+A.Location, ChargeDir*80000.f, KFM.CurrentDamType);
			WhatToDoNext(19);
		}
	}
	function EndState()
	{
		...
		{
			Doom3KF_LostSoul(Pawn).EndCharge();
			SetCombatTimer();
		}
	}
Begin:
