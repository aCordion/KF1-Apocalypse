class Doom3KF_Forgotten extends Doom3KF_LostSoul;

simulated function PostBeginPlay()
{
	Super(Doom3KF_DoomMonster).PostBeginPlay();
 	if (Level.NetMode != NM_DedicatedServer)
	{
		LostTrail = Spawn(class'Doom3KF_ForgottenTrailEffect', Self);
...
{
	LifeSpan = 0.6f;
	Spawn(class'Doom3KF_ForgottenExplosion');
	bHidden = true;
	if (LostTrail != None)
		LostTrail.Kill();
}
