...
//
//-----------------------------------------------------------
class GoreShank_ZombieGoreShank extends GoreShank_ZombieGoreShankBase
    Config(ApocZEDPack);
...
function RangedAttack(Actor A)
{
    ...
    if( !bShotAnim && !bDecapitated && VSize(A.Location-Location)<=900 )
...
state RunningState
{
    ...
    function BeginState()
    {
        GroundSpeed = OriginalGroundSpeed * 2.175;
        bRunning = true;
...
CheckCharge:
    ...
    if( Controller!=None
        ...
        && VSize(Controller.Target.Location-Location)<900 )
    {
        Sleep(0.5+ FRand() * 0.5);
        //log("Still charging");
        GoTo('CheckCharge');
...
CheckCharge:
    ...
    if( bZedUnderControl || (Controller!=None && Controller.Target!=None && VSize(Controller.Target.Location-Location)<900) )
    {
        Sleep(0.5+ FRand() * 0.5);
        GoTo('CheckCharge');
    }
...
simulated function int AttackAndMoveDoAnimAction( name AnimName )
{
    ...
    if( AnimName=='GoreAttack1' || AnimName=='GoreAttack2' || AnimName=='GoreAttack3' )
    {
...
defaultproperties
{
     ...
     Health=300
     ControllerClass=Class'GoreShank_GoreShankController'
     AmbientSound=SoundGroup'KF_EnemiesFinalSnd.GoreFast.Gorefast_Idle'
     Mesh=SkeletalMesh'GoreShankAnim.GoreShank'
     Skins(0)=Combiner'KF_Specimens_Trip_T.gorefast_cmb'
     Skins(1)=Texture'22CharTex.GibletsSkin'
     Skins(2)=Texture'22CharTex.SeveredSkin'
}
