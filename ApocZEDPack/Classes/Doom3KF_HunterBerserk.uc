...
simulated function PostBeginPlay()
{
    ...
    {
        BodyEffect = Spawn(class'Doom3KF_HunterMainEffect', self);
...
simulated function SetAnimAction(name NewAction)
{
    ...
    if (NewAction==SightAnim && Level.NetMode != NM_DedicatedServer)
    {
        ...
        ChargingFX = Spawn(Class'Doom3KF_HunterBerserkEffect', Self);
    }
...
simulated function SpawnAttackEffect()
{
    if (Level.NetMode != NM_DedicatedServer)
    {
        ChargeEffect = Spawn(class'Doom3KF_HunterChargeEffect', self, , );
...
{
    if (Damage>0 && !IsHeadShot(HitLocation, Normal(Momentum), 1.f))
        Damage = Max(Damage/4, 1);
...
function bool IsHeadShot(vector loc, vector ray, float AdditionalScale)
{
    ...
    if (!bIsRageMode || (Controller.MoveTarget != None && DesiredSpeed>0))
        return false;
...
defaultproperties
{
     ...
     FadeClass=Class'Doom3KF_HunterBerserkMaterialSequence'
     RangedProjectile=Class'Doom3KF_HunterProjectile'
     ...
     HealthMax=7500.00000 //3500.000000 kyan
     Health=7500 //3500 kyan
